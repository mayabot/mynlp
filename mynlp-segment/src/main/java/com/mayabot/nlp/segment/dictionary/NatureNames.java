/*
 * Copyright 2018 mayabot.com authors. All rights reserved.
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 *  you may not use this file except in compliance with the License.
 *  You may obtain a copy of the License at
 *
 *       http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

package com.mayabot.nlp.segment.dictionary;

/**
 * 参考HanLP https://github.com/hankcs/HanLP 中的实现
 *
 * @author jimichan
 */
public class NatureNames {
    public final static String xx = "xx";
    public final static String nic = "nic";
    public final static String ude2 = "ude2";
    public final static String ude3 = "ude3";
    public final static String ryt = "ryt";
    public final static String rys = "rys";
    public final static String ude1 = "ude1";
    public final static String yg = "yg";
    public final static String ryv = "ryv";
    public final static String qg = "qg";
    public final static String nit = "nit";
    public final static String nis = "nis";
    public final static String pbei = "pbei";
    public final static String nrf = "nrf";
    public final static String ad = "ad";
    public final static String qt = "qt";
    public final static String wky = "wky";
    public final static String qv = "qv";
    public final static String ag = "ag";
    public final static String nrj = "nrj";
    public final static String wkz = "wkz";
    public final static String rzs = "rzs";
    public final static String al = "al";
    public final static String nba = "nba";
    public final static String zg = "zg";
    public final static String an = "an";
    public final static String rzt = "rzt";
    public final static String nbc = "nbc";
    public final static String rzv = "rzv";
    public final static String rg = "rg";
    public final static String nbp = "nbp";
    public final static String begin = "begin";
    public final static String nsf = "nsf";
    public final static String rr = "rr";
    public final static String bg = "bg";
    public final static String ry = "ry";
    public final static String rz = "rz";
    public final static String vshi = "vshi";
    public final static String bl = "bl";
    public final static String uzhi = "uzhi";
    public final static String uyy = "uyy";
    public final static String Rg = "Rg";
    public final static String uzhe = "uzhe";
    public final static String ntc = "ntc";
    public final static String cc = "cc";
    public final static String a = "a";
    public final static String b = "b";
    public final static String nth = "nth";
    public final static String c = "c";
    public final static String d = "d";
    public final static String e = "e";
    public final static String f = "f";
    public final static String g = "g";
    public final static String nto = "nto";
    public final static String h = "h";
    public final static String i = "i";
    public final static String j = "j";
    public final static String k = "k";
    public final static String nts = "nts";
    public final static String l = "l";
    public final static String m = "m";
    public final static String ntu = "ntu";
    public final static String n = "n";
    public final static String o = "o";
    public final static String p = "p";
    public final static String udeng = "udeng";
    public final static String q = "q";
    public final static String r = "r";
    public final static String tg = "tg";
    public final static String s = "s";
    public final static String t = "t";
    public final static String u = "u";
    public final static String v = "v";
    public final static String w = "w";
    public final static String x = "x";
    public final static String y = "y";
    public final static String z = "z";
    public final static String dg = "dg";
    public final static String nmc = "nmc";
    public final static String dl = "dl";
    public final static String usuo = "usuo";
    public final static String ud = "ud";
    public final static String ug = "ug";
    public final static String uj = "uj";
    public final static String ntcb = "ntcb";
    public final static String ul = "ul";
    public final static String ntch = "ntch";
    public final static String mg = "mg";
    public final static String ntcf = "ntcf";
    public final static String uguo = "uguo";
    public final static String uv = "uv";
    public final static String mq = "mq";
    public final static String uz = "uz";
    public final static String nnd = "nnd";
    public final static String vd = "vd";
    public final static String pba = "pba";
    public final static String ule = "ule";
    public final static String vf = "vf";
    public final static String vg = "vg";
    public final static String vi = "vi";
    public final static String nb = "nb";
    public final static String nnt = "nnt";
    public final static String vl = "vl";
    public final static String nf = "nf";
    public final static String vn = "vn";
    public final static String udh = "udh";
    public final static String ng = "ng";
    public final static String nh = "nh";
    public final static String Mg = "Mg";
    public final static String ni = "ni";
    public final static String uls = "uls";
    public final static String nl = "nl";
    public final static String nm = "nm";
    public final static String nn = "nn";
    public final static String vx = "vx";
    public final static String nr = "nr";
    public final static String ns = "ns";
    public final static String nt = "nt";
    public final static String nx = "nx";
    public final static String wb = "wb";
    public final static String nz = "nz";
    public final static String wd = "wd";
    public final static String wf = "wf";
    public final static String wh = "wh";
    public final static String wj = "wj";
    public final static String wyy = "wyy";
    public final static String wm = "wm";
    public final static String wn = "wn";
    public final static String wyz = "wyz";
    public final static String wp = "wp";
    public final static String end = "end";
    public final static String gb = "gb";
    public final static String gc = "gc";
    public final static String ws = "ws";
    public final static String wt = "wt";
    public final static String vyou = "vyou";
    public final static String gg = "gg";
    public final static String ww = "ww";
    public final static String gi = "gi";
    public final static String gm = "gm";
    public final static String gbc = "gbc";
    public final static String gp = "gp";
    public final static String nhd = "nhd";
    public final static String nr1 = "nr1";
    public final static String nr2 = "nr2";
    public final static String nhm = "nhm";
    public final static String ulian = "ulian";
    public final static String xu = "xu";

}
